...
//Função que retorna 10 perguntas aleatorias.

function selecionarQuestoesAleatorias(questoes, quantidadeQuestoes){
    const questoesAleatorias = questoes.sort(() => Math.random() - 0.5);//Embaralha as perguntas do array questões
    return questoesAleatorias.slice(0, quantidadeQuestoes);//Retorna um novo array com 10 perguntas aleatórias selecionadas
}

//Função que exibe a pergunta e retorna a resposta do usuário.
...
function validarRespostaDoUsuario(repostaDoUsuario, pergunta){
    return repostaDoUsuario === pergunta.resposta;
}

//Funçao que retorna uma mensagem com base na pontuaçao do jogador.
function exibirResultado(pontuacaoFinal, nomeJogador){
    let mensagem = "";
    if(pontuacaoFinal <= 3){
        mensagem = "Oh não! Tente novamente.";
    }else if(pontuacaoFinal <= 6){
        mensagem ="Bom tentativa! Pratique um pouco mais.";
    }else if(pontuacaoFinal <= 9){
        mensagem ="Muito bom! Tu acertou a maioria.";
    }else{
        mensagem = "Excelente! Tu é um verdadeiro expert.";
    }
...
function iniciarQuiz(){
    ...
    const questoesSelecionadas = selecionarQuestoesAleatorias(questoes, quantidadeQuestoes);//Armazena as 10 questões aleatórias
